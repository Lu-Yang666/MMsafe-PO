...
from all_utils.data_utils.data_utils_ppo import preprocess_ppo_model, preprocess_ppo_model_image2
...
class QueryChosenRejectedResponseDataset2(Dataset):
    """Dataset that emits tokenized left-padded queries."""
    ...
    def __init__(
        ...
        self.data_args = data_args
        self.tokenizer = tokenizer ###
        super(QueryChosenRejectedResponseDataset2, self).__init__()
        ...
        for query, data in zip(queries, list_dict_data):
            if len(query) <= query_len:
                ...
                if (len(query) + len(chosen_response) < (self.tokenizer.model_max_length - 576)) and (len(query) + len(rejected_response) < (self.tokenizer.model_max_length -576)):
                    ...
                    truncate_length = len(query) + len(rejected_response) + 1 - (self.tokenizer.model_max_length -576)
                    rejected_response = rejected_response[:len(rejected_response)-truncate_length]
                    if (len(query) + len(chosen_response) < (self.tokenizer.model_max_length - 576)) and (len(query) + len(rejected_response) < (self.tokenizer.model_max_length -576)):
                        filtered_queries.append(query)
                        filtered_list_dict_data.append(data)
                        filtered_chosen_responses.append(chosen_response)
...
def make_dpo2_data_module(
    ...
    train_dataset = QueryChosenRejectedResponseDataset2(
        list_dict_data=list_data_dict,
        tokenizer=tokenizer,
        query_len=training_args.query_len, 
        data_args=data_args,
