...
from all_utils.data_utils.data_utils_dpo import make_dpo2_data_module
...
class DataArguments:
    ...
    config_train_path: str = field(default="config_train.json")
...
def check_param_dtype(model):
    print("\nChecking Base Model Parameters:")
    for name, param in model.base_model.named_parameters():
        dtype_list.append(param.dtype)
        if param.dtype != torch.bfloat16:
            param.data = param.data.to(torch.bfloat16)
            print(f"Non-BF16 Parameter: {name}, Dtype: {param.dtype}")

    # 打印 dtype 列表和唯一的 dtype 数量
    unique_dtypes = set(dtype_list)
    print("\nDtype List:", unique_dtypes)
    print("Number of Unique Parameter Types:", len(unique_dtypes))
...
def train():
    ...
    tokenizer_model_name = args.policy_model_name_or_path
    ...
    check_param_dtype(model=policy)
    check_param_dtype(model=ref_policy)
    ...
    # Dataset
    data_module: dict = make_dpo2_data_module(
